...
const Index = ({ orders, products }) => {
  ...
  const handleDelete = async (id) => {
    ...
    //     `https://angry-swartz-81a245.netlify.app/api/products/${id}`
    //   );
  ...
  const handleStatus = async (id) => {
    ...
    if (data.status <= 2) {
      try {
        const response = await axios.put(
          `https://angry-swartz-81a245.netlify.app/api/orders/${id}`,
          data
        );
...
export const getServerSideProps = async (ctx, window) => {
  ...
  const productResponse = await axios.get(
    `https://angry-swartz-81a245.netlify.app/api/products`
  );
  const orderResponse = await axios.get(`https://angry-swartz-81a245.netlify.app/api/orders`);
